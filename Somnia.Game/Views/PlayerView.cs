...
namespace Somnia.Game.Views
{
    public class PlayerView
    {
        ...
        public void DrawWorld(SpriteBatch sb, PlayerModel p, List<EnemyModel> enemies, List<AnomalyZone> zones, NpcModel npc)
        {
            ...
            // Если включена аура - рисуем постоянный круг вокруг игрока
            if (p.GreenAuraTimer > 0)
                DrawCircle(sb, p.Position + new Vector2(25, 25), 200f, Color.LimeGreen, 3);
        ...
        {
            Color c = GetZoneColor(p.CurrentZone);
            Vector2 cPos = p.Position + new Vector2(25, 25); 
            
            if (p.CurrentZone == AnomalyType.Neutral) {
                if (p.ActiveSlot == 0) DrawLine(sb, cPos, cPos + p.FacingDir * 1000f, c, 4); // Снайперка
                else if (p.ActiveSlot == 2) DrawLine(sb, cPos, cPos + p.FacingDir * 1000f, Color.LimeGreen, 6); // Выстрел вирусом
            }
        ...
        private void DrawEnemy(SpriteBatch sb, EnemyModel e)
        {
            ...
            if (e.IsDummy) eCol = Color.Orange;
            ...
            // Если заражен - рисуем зеленую точку над ХП баром
            if (e.IsInfected)
                sb.Draw(_tex, new Rectangle((int)e.Position.X + 15, (int)e.Position.Y - 20, 10, 10), Color.LimeGreen);
        }
        ...
        private void DrawCircle(SpriteBatch sb, Vector2 center, float r, Color c, int t=2) {
            ...
            Vector2 p1 = center + new Vector2((float)System.Math.Cos(th), (float)System.Math.Sin(th)) * r;
            for (int i = 0; i < 32; i++) {
                th += inc; Vector2 p2 = center + new Vector2((float)System.Math.Cos(th), (float)System.Math.Sin(th)) * r;
                DrawLine(sb, p1, p2, c, t); p1 = p2;
            }
        }
        private void DrawCone(SpriteBatch sb, Vector2 c, Vector2 d, float r, float aStr, Color col) {
            float bA = (float)System.Math.Atan2(d.Y, d.X); float s = (float)System.Math.Acos(aStr);
            Vector2 p1 = c + new Vector2((float)System.Math.Cos(bA-s), (float)System.Math.Sin(bA-s)) * r;
            Vector2 p2 = c + new Vector2((float)System.Math.Cos(bA+s), (float)System.Math.Sin(bA+s)) * r;
