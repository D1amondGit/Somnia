...
namespace Somnia.Game.Views
{
    public class PlayerView
    {
        ...
        public void DrawWorld(SpriteBatch sb, PlayerModel p, List<EnemyModel> enemies, List<AnomalyZone> zones, NpcModel npc, Rectangle hatch, Rectangle playArea, List<Vector3> walls, List<ProjectileModel> projs)
        {
            // 1. Цветные зоны
            foreach (var z in zones) sb.Draw(_tex, z.Area, GetZoneColor(z.Type) * 0.25f);
            
            // 2. ГРАНИЦЫ КАРТЫ (Blackout по краям экрана)
            sb.Draw(_tex, new Rectangle(0, 0, 1280, playArea.Y), Color.Black); // Верх
            sb.Draw(_tex, new Rectangle(0, playArea.Bottom, 1280, 720 - playArea.Bottom), Color.Black); // Низ
            sb.Draw(_tex, new Rectangle(0, playArea.Y, playArea.X, playArea.Height), Color.Black); // Лево
            sb.Draw(_tex, new Rectangle(playArea.Right, playArea.Y, 1280 - playArea.Right, playArea.Height), Color.Black); // Право
            
            // 3. ЛЮК (Синий залитый круг)
            Color hatchCol = p.State == PlayerState.Carrying ? Color.Cyan : Color.DarkBlue;
            FillCircle(sb, new Vector2(hatch.X + hatch.Width/2, hatch.Y + hatch.Height/2), 40f, hatchCol); 
            
            // 4. СТЕНЫ (Идеально залитые черные круги)
            foreach(var w in walls) {
                FillCircle(sb, new Vector2(w.X, w.Y), w.Z, Color.Black);
            }

            // ПУЛИ СТРЕЛКОВ
            foreach (var pr in projs) FillCircle(sb, pr.Position, 8f, Color.Red);
        ...
        private void DrawEnemy(SpriteBatch sb, EnemyModel e) {
            ...
            // Обводка для дальника (как на скетче)
        ...
        public void DrawPauseMenu(SpriteBatch sb, SpriteFont f, int w, int h) {
            sb.Draw(_tex, new Rectangle(0, 0, w, h), Color.Black * 0.8f);
            if (f != null) {
        ...
        // ИДЕАЛЬНАЯ ЗАЛИВКА КРУГА (Без дырок!)
        private void FillCircle(SpriteBatch sb, Vector2 center, float radius, Color color) {
            for (float y = -radius; y <= radius; y++) {
                float width = (float)Math.Sqrt(radius * radius - y * y) * 2f;
                sb.Draw(_tex, new Rectangle((int)(center.X - width/2), (int)(center.Y + y), (int)width, 1), color);
            }
        }

        private void DrawLine(SpriteBatch sb, Vector2 p1, Vector2 p2, Color c, int t=2) {
            Vector2 e = p2 - p1; float angle = (float)System.Math.Atan2(e.Y, e.X);
