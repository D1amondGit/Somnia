...
namespace Somnia.Game.Models
{
    ...
    public class EnemyModel
    {
        ...
        public EnemyModel(Vector2 pos, EnemyType type = EnemyType.Melee) { Position = pos; Type = type; }
        
        public void TakeDamage(float dmg, Vector2 src, float kb) {
            Health -= dmg; 
            DamageFlash = 0.15f; // Белая вспышка при уроне
            Vector2 dir = Position - src;
            if (dir != Vector2.Zero && kb > 0) { dir.Normalize(); Velocity += dir * kb; } // Отдача
        }
        
        public void Update(float dt) {
            Position += Velocity * dt; 
            Velocity = Vector2.Lerp(Velocity, Vector2.Zero, 0.1f); // Затухание толчка
